...
- **Watch mode (new / modified)**: track only newly created tickets, or also detect changes to existing ones (status, comments…). A **popup filter (All / New)** lets you show only new detections.
...
- **Mode de surveillance (nouveaux / modifiés)** : suivre uniquement les nouveaux tickets, ou détecter aussi les modifications des tickets existants (statut, commentaires…). Un **filtre popup (Tous / Nouveaux)** permet de n'afficher que les nouvelles détections.
